        ...
        <div className="space-y-5">
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-lg p-4">
                <div className="flex items-center gap-2 mb-2">
                    <span className="text-sm font-medium text-slate-700">模板（内置）</span>
            ...
            <div>
                ...
                <div className="text-xs text-slate-500 mb-3">用于计算字段分布特征，不影响语义裁决与质量结果</div>
            ...
            <div className="border-t border-slate-200 pt-4 mt-2">
                <div className="text-xs font-semibold text-slate-500 mb-3 uppercase tracking-wider">系统执行参数（只读）</div>
                <div className="bg-slate-50 border border-slate-200 rounded-lg p-3 text-xs text-slate-500 space-y-2">
                    <div className="flex justify-between">
                        <span>最大采样行数</span>
                        <span className="font-mono">{assist.systemConfig.maxRows.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between">
                        <span>缓存有效期 (TTL)</span>
                        <span className="font-mono">{assist.systemConfig.ttlHours}h</span>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
                <div className="bg-white rounded-xl shadow-2xl w-[480px] overflow-hidden">
                    <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
                        <h3 className="text-base font-bold text-slate-800">语义理解辅助检测 · 本次运行</h3>
                        <button
                            onClick={onClose}
